...
func (m *rbiModule) InitContext(c pgs.BuildContext) {
	...
	funcs := map[string]interface{}{
		...
		"rubyMessageTypeWithoutNamespace": ruby_types.RubyMessageTypeWithoutNamespace,
...
module {{ rubyPackage .File }}Validator
{{ range .AllMessages }}

{{ $message := . }}
    class {{ rubyMessageTypeWithoutNamespace . }}Validators
      extend T::Sig
      include Validation
